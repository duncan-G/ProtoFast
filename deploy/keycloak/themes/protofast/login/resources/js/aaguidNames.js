/* Passkey-provider AAGUIDs → the name the provider is known by.

   GENERATED — do not hand-edit. Regenerate with:

     curl -sSL https://raw.githubusercontent.com/passkeydeveloper/passkey-authenticator-aaguids/main/aaguid.json \
       | jq -S 'to_entries | map({(.key): .value.name}) | add'

   Source: github.com/passkeydeveloper/passkey-authenticator-aaguids, the
   community list of *passkey provider* AAGUIDs — iCloud Keychain, Google
   Password Manager, Windows Hello, 1Password and the like. Its sibling
   `combined_aaguid.json` also folds in the FIDO Metadata Service, which names
   every certified hardware key; that half is deliberately left out.

   The realm asks for `attestation: none`
   (webAuthnPolicyPasswordlessAttestationConveyancePreference in
   protofast-realm.json), and under `none` browsers zero the AAGUID of a
   roaming security key. The provider AAGUIDs above still come through, because
   a synced-passkey provider reports its identity outside the attestation
   statement. So the MDS half would be 330 more entries that can never match.
   If that policy ever moves to `indirect` or `direct`, regenerate from
   `combined_aaguid.json` instead and this lookup starts naming hardware keys.

   One entry is dropped by hand: 6d212b28-a2c1-4638-b375-5932070f62e9, whose
   upstream name is the literal string "initial".

   Names are shown to the user as the suggested passkey label, but they are
   vendor product names, not copy — they are not translated and do not belong
   in messages_en.properties. */
export const AAGUID_NAMES = Object.freeze({
