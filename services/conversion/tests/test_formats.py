"""Dispatch: which handler a request reaches, and which requests reach none (ingest plan §6)."""
...
@pytest.mark.parametrize(
    ("key", "media_type"),
    [
        # The extension is not on the list.
        ...
        # The two halves disagree: `api` never signed a policy for this pair.
...
def test_an_absent_media_type_falls_back_to_the_extension():
    # The worker always sends one, but treating an empty string as "no opinion" keeps the service
    # usable by hand during an incident.
