"""Layout emission and the OCR paragraph reconstruction (ingest plan §11)."""
...
def test_the_json_shape_is_the_one_raw_layout_line_deserialises():
...
def test_weight_and_slant_are_read_from_the_font_name():
    # Embedded subsets are named like "ABCDEF+Helvetica-BoldOblique"; there is no weight axis in a
    # PDF font reference, so a substring test is all the information there is.
...
def test_ocr_lines_wrap_into_paragraphs_at_wide_gaps():
    lines = [
        ...
        # A gap wider than a line of text: a new paragraph, not a wrap.
...
def test_no_recognised_text_produces_no_markdown():
    # An empty string is what the caller turns into a "no readable text" refusal; inventing a
    # placeholder here would send an empty document down the pipeline instead.
    assert to_markdown([]) == ""
