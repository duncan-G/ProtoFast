"""The HTTP surface (ingest plan §9).

The status codes are the contract, not a detail: the caller reads 4xx as "this document will never
convert, fail the run now" and 5xx as "try again". Getting that split wrong either burns five SQS
delivery attempts on a malformed PDF or fails a run that a redeploy would have fixed.
"""
