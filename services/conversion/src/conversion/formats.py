"""The accepted source formats, mirroring ``SourceFormats.cs`` (ingest plan §6).

Two copies of one allowlist is a drift hazard, so ``tests/test_format_parity.py`` reads the C#
table and asserts this one equals it, row for row. The duplication itself is deliberate: the
converter must be able to refuse a format on its own, because a valid presigned POST pins the key
and the declared content type but not the *bytes* written under them.
"""
...
def extension_of(key: str) -> str | None:
    """The extension of an S3 key, lowercased, or None when it has none.

    Only the last path segment is considered, so a key can never resolve through a ``..``.
    """
    name = key.rsplit("/", 1)[-1]
    dot = name.rfind(".")
...
def resolve(source_key: str, media_type: str) -> Format | None:
    """The format for this request, or None when the two halves disagree.

    Both are checked, and against each other: ``api`` signed the policy for one particular pair,
    so a request naming a different pair did not come from a URL ``api`` minted.
    """
