...
public class DagTests
{
    ...
    {
        var e = Assert.Throws<InvalidWorkflowException>(() => Dag.Validate(Workflow(Stage("a", "b"), Stage("b", "a"))));
        Assert.Contains("cycle", e.Message);
    }

    [Fact]
    public void Validate_rejects_an_unknown_dependency() =>
        Assert.Throws<InvalidWorkflowException>(() => Dag.Validate(Workflow(Stage("a", "missing"))));

    [Fact]
    public void Validate_rejects_a_duplicate_stage() =>
        Assert.Throws<InvalidWorkflowException>(() => Dag.Validate(Workflow(Stage("a"), Stage("a"))));

    [Fact]
    public void Validate_rejects_the_reserved_input_stage_id() =>
        Assert.Throws<InvalidWorkflowException>(() => Dag.Validate(Workflow(Stage(ArtifactRef.InputStageId))));
