    ...
    TimeProvider time) : IRegistry
{
    ...
    {
        var bytes = EngineJson.SerializeToUtf8Bytes(content);
        var hash = Hash(bytes);
        ...
        // Serializes publishers of the same id, so reading the latest version and inserting the
        // next one cannot interleave. Released at commit.
        var lockKey = $"{kind}:{id}";
        await db.Database.ExecuteSqlInterpolatedAsync($"SELECT pg_advisory_xact_lock(hashtextextended({lockKey}, 0))", ct);

        var version = 1 + (await db.RegistryEntries
    ...
    private async Task<T> ReadAsync<T>(string key, CancellationToken ct)
    {
        ...
        return EngineJson.Deserialize<T>(bytes);
    }
