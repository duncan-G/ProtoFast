...
| A secret value | `scripts/populate-secrets.sh Key=value` (prod) or `--dev` (local) | deploy the consuming component (prod) / restart aspire (dev) |
...
| `Auth_Keycloak__ClientSecretProtofastWeb` / `…Admin` / `AdminClientSecret` | `appsettings.Development.json`, overridable from `protofast/dev` | Secrets Manager → compose `.env` |
