...
`KEYCLOAK_DOMAIN`, `DOCUMENTS_BUCKET` (from the deploy job), plus the secret-derived values
`AUTH_DB_PASSWORD`, `PROTOFAST_DB_PASSWORD`,
...
Variables: `AWS_REGION`, `TFSTATE_BUCKET`, `ASSETS_BUCKET`, `DOCUMENTS_BUCKET`.
