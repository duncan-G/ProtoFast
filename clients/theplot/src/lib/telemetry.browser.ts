 ...
 *
 * The `documentLoad` span joins the trace of the SSR render that produced the page: the
 * server writes its traceparent into `<meta name="traceparent">` (see telemetry.ssr.ts) and
 * DocumentLoadInstrumentation parents the page-load spans on it. Requests the app makes after
 * that each start their own trace.
 */
