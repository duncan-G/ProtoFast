...
@Component({
  ...
  imports: [AccountMenu, DatePipe, RouterLink, TheplotLogo],
  ...
  template: `
    ...
    <div
      class="ember flex min-h-screen flex-col bg-[radial-gradient(1100px_520px_at_12%_-12%,color-mix(in_srgb,var(--color-accent)_11%,transparent),transparent_62%)] antialiased"
    >
      <header>
        <nav class="mx-auto flex w-full max-w-[700px] items-center px-6 py-[17px] sm:px-8">
          <a routerLink="/app" class="mr-auto no-underline"><app-theplot-logo /></a>
      ...
      <main class="mx-auto w-full max-w-[700px] flex-1 px-6 pb-[100px] sm:px-8">
        @if (deleted()) {
          ...
          >
            ...
            <a href="/" rel="external" class="btn btn-secondary mt-1">Back to ThePlot</a>
          </div>
        } @else {
...
const ENROLMENT_SNAPSHOT_KEY = 'theplot.account.passkeys-before-enrolment';
